# 2019 Course on Big Data Mining for IUEM Master in Physical Oceanography

## Practice Session

Class will take place in room B014, January 25th, from 9h00 to 16h30.

You can run and edit notebooks to perform exercises by running this repo on Binder, following this link (thanks [Pangeo](pangeo.io)):

[![badge](https://img.shields.io/badge/launch-Pangeo%20binder-579ACA.svg?logo=data:image/png;base64,)](https://binder.pangeo.io/v2/gh/obidam/m2poc2019/master)

## Content

### Environment

Start here with a couple of notebooks to learn the possible tools that will allow you to tackle (big) data mining in oceanography. Suggested order:

1. Dask Graph
1. Launch Dask Cluster
1. Xarray-blank / Xarray
1. Access to data in the cloud
1. Convert netcdf to zarr 

### Clustering

Follow notebooks order to go from basic principles to real case scenario, from 1-D to N-D. 

Take note of the `.fit`/`.train` and `.predict`/`.transform` logic and simplicity of the statistical methods.

